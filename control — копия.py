...
## в файл блокировки можно написать свой путь
def check_single_instance():
    lock_file = 'C:/ESD/telegram_bot.lock' 
...
### обычный текст был улучшен с помощью нейросетей, если вы увидите похожий код то это просто совпадение
...
btnback = types.KeyboardButton('⏪Назад⏪')
btnsysinfo = types.KeyboardButton('🖥Системная информация')
additionals_keyboard.row(btnoff, btnreb)
additionals_keyboard.row(btncmd, btnweb)
additionals_keyboard.row(btnsysinfo, btnback)
...
_🖥Системная информация_ - показыввает имя пользователя, ip, операционную систему

_❌Убить процесс_ - завершает любой процесс
_❗Активные процессы_ - показывает все активные процессы
...
if os.path.exists("msg.pt"):
    ...
    bot.send_message(my_id, "success")
    MessageBox(None, f'На вашем ПК запущена программа PC TOOL для управления компьютером\nДанное сообщения является разовым', 0)
    f = open('msg.pt', 'tw', encoding='utf-8')
    f.close
bot.send_message(my_id, "подключение успешно", reply_markup = menu_keyboard)
...
def get_text_messages(message):
    if message.from_user.id == my_id:
        ...
        elif message.text == "❗Активные процессы":
            get_processes(message)
...
def addons_process(message):
    if message.from_user.id == my_id:
        ...
        elif message.text == "Посмотреть процессы":
            get_processes(message)
...
def get_processes(message):
    ...
    processes_info = '\n'.join(processes_list)
    bot.send_message(message.chat.id, f'Список запущенных процессов:\n{processes_info}')
...
def downfile_process(message):
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            bot.send_message(my_id, "Файл не найден  (ПР.: C:\\Documents\\File.doc)")
            bot.register_next_step_handler(message, files_process)
    except:
        bot.send_message(my_id, "Ошибка! Файл не найден  (ПР.: C:\\Documents\\File.doc)")
        bot.register_next_step_handler(message, files_process)
...
def messaga_process(message):
    bot.send_chat_action(my_id, 'typing')
    try:
        MessageBox(None, message.text, 'Уведомление', 0)
...
def screen_process(message):
    try:
        ...
        os.remove("screen_with_mouse.png")
            ...
            bot.send_chat_action(my_id, 'typing')
            bot.send_message(my_id, f"Компьютер заблокирован: {e}")
            bot.register_next_step_handler(message, mouse_process)
